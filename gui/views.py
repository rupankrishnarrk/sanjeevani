...
class Home(LoginRequiredMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        gender_count = models.PatientProfileModel.objects.values('gender').annotate(count=Count('gender'))
        return render(request, self.template_name, {'reminder': reminder, 'gender_count': gender_count})
...
class Profile(LoginRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            return redirect(reverse("gui:profile", kwargs={'identifier': kwargs['identifier']}))
        print(form.errors)
...
class PatientProfileCreateView(LoginRequiredMixin, FormView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, self.template_name, {'profile_base64': profile_base64, 'my_referrals': my_referrals})
...
class CalendarView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        endTime = datetime.now() + timedelta(days=180)
        appointment = models.AppointmentModel.objects.filter(starttime__gte=startDate, starttime__lte=endTime)
        panchakarma = models.PatientScheduleModel.objects.filter(datetime__gte=startDate, datetime__lte=endTime)
        return render(request, self.template_name, {'appointment': appointment, 'panchakarma': panchakarma })
